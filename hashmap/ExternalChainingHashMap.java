...
public class ExternalChainingHashMap<K, V> {
    ...
    public V put(K key, V value) {
        ...
        // get the index of where to insert (key,value) pair
        int index = Math.abs(key.hashCode()%table.length);
        
        // If the index is null then insert it there
        if (table[index] == null) {
            table[index] = new ExternalChainingMapEntry(key, value);
        } else {
            // Get the entry residing at the index
            ExternalChainingMapEntry<K, V> entry = table[index];
            // start traveresing through all the items stored at that index until end is reached
            while(entry.getNext() != null && !entry.getKey().equals(key)) {
                entry = entry.getNext();
            }
            // If duplicate is found then return the old value as well as replace it with new value
            if (entry.getKey().equals(key)) {
                V oldValue = entry.getValue();
                entry.setValue(value);
                return oldValue;
            } else {
                ExternalChainingMapEntry<K, V> currentHead = table[index];
                table[index] = new ExternalChainingMapEntry(key, value);
                table[index].setNext(currentHead);
            }
        }
        size++;
        return null;
    ...
    private double currentLoadFactor() {
        return (size + 1)/(double)table.length;
    }
    ...
    public V remove(K key) {
        ...
        int index = Math.abs(key.hashCode()%table.length);
        // If item exist at index then continue or else throw NoSuchElementException
        if (table[index] != null) {
